...
if submit_button:
    ...
    if not data.empty:
        ...
        fig.add_trace(go.Scatter(mode='markers', x=data[data['Crossover'] == 1].index, y=data['Close'], marker_symbol='triangle-up', marker_color='green', marker_size=10, name='Bullish Crossover ✅'), row=1, col=1)
        fig.add_trace(go.Scatter(mode='markers', x=data[data['Crossover'] == -1].index, y=data['Close'], marker_symbol='triangle-down', marker_color='red', marker_size=10, name='Bearish Crossover 🈲'), row=1, col=1)
